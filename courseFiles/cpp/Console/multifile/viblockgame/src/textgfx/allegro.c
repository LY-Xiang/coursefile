...
static void load_pc8x16_font()
{
	...
	if (p) {
		...
		append_filename(fname, fname, "viblockgame", sizeof(fname));
		set_allegro_resource_path(5, fname);
	}
#ifdef UNIX
	set_allegro_resource_path(4, "/usr/share/allegro/viblockgame");
	set_allegro_resource_path(3, "/usr/share/allegro");
	set_allegro_resource_path(2, "/usr/local/share/allegro/viblockgame");
...
{
#ifdef UNIX
	strcpy(_xwin.application_name, "viblockgame");
	strcpy(_xwin.application_class, "Viblockgame");
#endif
...
#endif
	load_pc8x16_font();
	set_window_title(VIBLOCKGAME_VER);
