...
struct blockgame2p blockgame2p[2];
...
static int
draw_lineclear_step(struct player *plr, struct blockgame2p *tet, int first)
{
...
static void sendgarbage(struct blockgame2p *tet)
{
	...
	if (n > 0) {
		i = tet==blockgame2p;
		blockgame2p[i].garbage[0] += n;
		if (blockgame2p[i].garbage[0] > 12)
			blockgame2p[i].garbage[0] = 12;
		upd_garbagemeter(game->player+i, blockgame2p[i].garbage[0]);
	}
}
...
static void spawndelay(struct player *plr, struct blockgame2p *tet)
{
	tet->falltm = plr->falltime;
	tet->delay  = SPAWN_DELAY;
	if (!NETPLAY || tet==blockgame2p)
...
static void fix_delayed_garbage(int n)
{
	...
	if (n) {
		...
		if (p->blocks == TETR_I)
			...
			n++;
		if (n < 0)	/* game over */
			blockgame2p[1].delay = 1;
	}
}

void getgarbage_player2()
{
	char *garbg = blockgame2p[1].garbage;
	int n = garbg[1];
...
static void lockdelay(struct player *plr, struct blockgame2p *tet)
{
...
static int upd_lockdelay_state(struct player *plr, struct blockgame2p *tet)
{
	struct tetr *p = &plr->piece;
	if (!hitbtm(p, plr)) {
		tet->delay = 0;
...
static void lockpiece_2p(struct player *plr, struct blockgame2p *tet)
{
	if (NETPLAY) {
		if (tet>blockgame2p && plr->piece.blocks) {
			lockdelay(plr, tet);
			return;
		}
		if (!tet->b1)
...
{
	struct blockgame2p *tet = blockgame2p;
...
{
	int tm = (blockgame2p[0].delay) ? blockgame2p[0].delay : blockgame2p[0].falltm;
	if (blockgame2p[1].delay) {
		if (tm > blockgame2p[1].delay)
			tm = blockgame2p[1].delay;
	} else if (tm > blockgame2p[1].falltm)
		tm = blockgame2p[1].falltm;
#ifdef SOCKET
	if (tm > 1000)
		tm = 1000;
#endif
...
static int upd_player(struct player *plr, struct blockgame2p *tet, int tm)
{
	if (tet->delay) {
		tet->delay -= tm;
		if (tet->falltm)
...
{
	memset(blockgame2p, 0, 2*sizeof(struct blockgame2p));
	winner = NULL;
	if (!NETPLAY || is_server) {
		gen_tetrom_seq(0, 8);
...
static int gameover_timeout(int *gameover)
{
	...
	if (*gameover == 1 && blockgame2p[0].delay < 31768 && blockgame2p[1].b1)
		return 1;
	if (*gameover == 3 && blockgame2p[1].delay < 31768) {
		*gameover = 1 + (blockgame2p[0].delay >= blockgame2p[1].delay);
		return 1;
	}
	return 0;
#endif
...
static void sync_spawn_delay()
{
	if (NETPLAY && tetrom_seq_pos[0] == tetrom_seq_pos[1] &&
	    blockgame2p[0].delay > 100 && blockgame2p[1].delay > 100 &&
	    blockgame2p[0].falltm && blockgame2p[1].falltm == player1.falltime)
	{
		blockgame2p[0].delay = blockgame2p[1].delay;
	}
}
...
static void set_gameover(int *gameover, int i)
{
	*gameover |= 1+i;
	if (NETPLAY && !is_server) {
		blockgame2p[i].falltm = -1;
		blockgame2p[i].delay = 0x7FFF;
	}
}
...
static void updateplayer(int *gameover, int i, int t)
{
	...
	if (!upd_player(plr, blockgame2p+i, gettm(t)-t) && !nextpiece_2p(plr))
...
static struct player *play_round()
{
	...
	while (!gameover || !gameover_timeout(&gameover)) {
		...
		if (ret == -2)
			return NULL;
		if (NETPLAY && i) {
			if (!blockgame2p[1].delay && blockgame2p[1].garbage[1]) {
				blockgame2p[1].delay = 0x7FFF;
		...
		if (!ret)
			lockpiece_2p(game->player+i, blockgame2p+i);
		if (ret)
