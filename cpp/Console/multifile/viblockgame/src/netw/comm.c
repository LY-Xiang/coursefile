...
static void recvpiece()
{
	...
	if (blockgame2p[1].delay) {
		if (*blockgame2p[1].clearedlines) {
			redrawboard(&player2, 19);
			*blockgame2p[1].clearedlines = 0;
		}
		if (!p->piece.blocks)
			nextpiece_2p(p);
	}
...
static int recvgarbage_num(int n)
{
	if (n <= 12)
		...
		blockgame2p[1].garbage[1] = n-12;
		sock_flags |= PL2_GARBAGE;
		if (!blockgame2p[1].delay || blockgame2p[1].falltm) {
			getgarbage_player2();
			return 1;
		}
	}
